...
public class TestPredicate extends TestTerm {
    ...
    private Implication pxImpliesPy = new Implication(px, py); // P(x) -> P(y)
    ...
    public void testImplies() { // (P(x) -> P(y)[x\y] = P(y) -> P(y)
        Expression pyImpliesPy = new Implication(py, py);
    ...
    public void testForAll3() { // (FORALL x . Q(x,y))[y\f(x)] = FORALL v0 . Q(v0,f(x)) 
        ...
        Expression expected = new ForAll(v0, new Predicate("Q", new TermList(v0, fx)));
